package newhome.baselibrary.ImageHandle.ImageUse.luban;
...
import static newhome.baselibrary.ImageHandle.ImageUse.luban.Preconditions.checkNotNull;
...
public class Luban {
    ...
    private File thirdCompress(@NonNull File file) {
        String thumb = FileUtil.appSavePath() +
    ...
    private File firstCompress(@NonNull File file) {
        ...
        String filePath = file.getAbsolutePath();
        String thumbFilePath = FileUtil.appSavePath() +
                (TextUtils.isEmpty(filename) ? System.currentTimeMillis() : filename) + ".jpg";
